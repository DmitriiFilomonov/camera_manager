export const icons = {
    vehicle: `
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5H15C15 3.9 14.1 3 13 3H11C9.9 3 9 3.9 9 5H6.5C5.84 5 5.29 5.42 5.08 6.01L3 12V20C3 20.55 3.45 21 4 21H5C5.55 21 6 20.55 6 20V19H18V20C18 20.55 18.45 21 19 21H20C20.55 21 21 20.55 21 20V12L18.92 6.01ZM6.5 16C5.67 16 5 15.33 5 14.5C5 13.67 5.67 13 6.5 13C7.33 13 8 13.67 8 14.5C8 15.33 7.33 16 6.5 16ZM17.5 16C16.67 16 16 15.33 16 14.5C16 13.67 16.67 13 17.5 13C18.33 13 19 13.67 19 14.5C19 15.33 18.33 16 17.5 16ZM5 11L6.5 6.5H17.5L19 11H5Z" fill="currentColor"/>
    </svg>
  `,
    wifi: (level) => {
        const levels = [
            `<svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path d="M1 9L2 10C5.6 6.6 10.4 6.6 14 10L15 9C10.9 5 5.1 5 1 9Z" fill="#ff4444"/>
        <path d="M5 13L6 14C7.7 12.3 10.3 12.3 12 14L13 13C10.8 10.8 7.2 10.8 5 13Z" fill="#ff4444"/>
        <circle cx="12" cy="18" r="2" fill="#ff4444"/>
      </svg>`
        ]
        return levels[level - 1] || levels[0]
    }
}
export function getWifiColor(level) {
    const colors = ['#ff4444', '#ff8844', '#ffcc44', '#aadd44', '#66cc44']
    return colors[level - 1] || colors[0]
}